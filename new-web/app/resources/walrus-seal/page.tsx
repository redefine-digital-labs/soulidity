...
export default function WalrusSealPage() {
  ...
  contentHash: string,    // hex — SHA-256 of plaintext; bound inside encryptedDek
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <p className="text-sm text-muted">
          The API server builds sidecars after on-chain TX success via <code>buildSyncSealSidecars</code> in <code>new-web/lib/soulidity/mirror/build-seal-sidecars.ts</code>. The server receives the raw DEK envelope (sealed at upload time), connects to the Seal key server network as a server-side client, and re-encrypts the DEK under the now-known on-chain object IDs.
        </p>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <ol className="text-sm text-muted space-y-1.5 list-decimal ml-5">
          ...
          <li>Build the approval transaction bytes matching the policy (owner vs granted-agent path).</li>
