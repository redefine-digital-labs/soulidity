    ...
    <div className="max-w-[760px] mx-auto px-6 py-8 relative z-10 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Soulidity exposes REST API endpoints for post-TX mirroring, soul browsing, and access resolution — plus a TypeScript SDK in <code>new-web/lib/soulidity/</code> for transaction building, on-chain queries, and client-side decryption.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <p className="text-sm text-muted">All routes are under <code>/api/souls/</code> in <code>new-web/app/api/souls/</code>.</p>
...
Authorization: Bearer snk_...`}</code>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <p className="text-sm text-muted">All SDK files live in <code>new-web/lib/soulidity/</code>.</p>
...
interface ActiveGrantSlotObject {
  ...
  expiresAtMs: number | null,
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <p className="text-sm text-muted">
          <code>new-web</code> uses two aliases relevant to Soulidity work:
        </p>
        <ul className="text-sm text-muted space-y-1">
          <li><code className="text-xs text-foreground">@/*</code> → <code>new-web/*</code> — local SDK, components, hooks</li>
          <li><code className="text-xs text-foreground">@web/*</code> → <code>web/*</code> — shared services (Walrus, Seal, Prisma, auth)</li>
        </ul>
        <p className="text-xs text-muted mt-1">
          The Prisma client is generated at <code>generated/prisma/</code> and imported via <code>@web/lib/prisma</code>. Run <code>npx prisma generate --schema=prisma/schema.prisma</code> after schema changes.
        </p>
      </div>
