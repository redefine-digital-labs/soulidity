...
import {
  markSkillVersionDeletedFromChain,
...
export async function POST(
  ...
  const stored = await getStoredSoulidityTxSync({
    ...
    resourceKey: `${soul.onChainId}:${versionId}`,
  })
  if (stored) {
  ...
  try {
    ...
    if (deleted.soulId !== soul.onChainId || deleted.versionId !== versionId.toLowerCase()) {
    ...
    await markSkillVersionDeletedFromChain({
      versionOnChainId: deleted.versionId,
    })
