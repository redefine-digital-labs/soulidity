...
const activationPolicies = [
  { value: 'public', label: 'Public', desc: 'Anyone can interact with this Soul layer', icon: '🌐' },
  { value: 'holder', label: 'Holder-only', desc: 'Restricted to current NFT holder', icon: '🔑' },
  { value: 'creator', label: 'Creator-only', desc: 'Only you can activate this Soul', icon: '🛡️' },
]

export default function PersonalConfigurePage() {
  ...
  return (
    <div className="relative z-10">
      <FlowBar steps={['Select NFT', 'Configure', 'Preview', 'Gas', 'Success']} current={1} />
      ...
      <div className="max-w-[540px] mx-auto px-6 py-8">
        ...
        <p className="text-muted text-sm mb-6">Define the Soul character, memory, and activation rules.</p>

        {/* Soul Character Upload */}
        <div className="mb-5">
          <label className="block text-xs font-semibold text-muted uppercase tracking-[0.08em] mb-1.5">
            Soul Character File <span className="text-danger">*</span>
          </label>
          <label className="block border-2 border-dashed border-border rounded-xl p-6 text-center cursor-pointer hover:border-purple hover:bg-purple/5 transition">
            ...
            {fileName ? (
              ...
              <div>
                <div className="font-semibold text-sm mb-1">Upload Soul Character (.md)</div>
                <div className="text-muted text-xs">Markdown character sheet. Max 1MB.</div>
              </div>
            )}
          </label>
        </div>
        ...
        <div className="mb-5">
          ...
          <textarea
            ...
            placeholder="Initial context for this Soul — backstory, knowledge, behavioral anchors..."
            value={memorySeed}
            onChange={(e) => setMemorySeed(e.target.value)}
          />
          <div className="text-right text-[11px] text-muted mt-1">{memorySeed.length} chars</div>
        </div>
