...
**总计：64 项测试（其中 63 项可直接执行 + 1 项需补 memory append helper），11 个 Phase**
...
**运行时变量：** `SOUL_A_ID`（Soul A on-chain ID），`SOUL_A_STATE_OBJ`（Soul A state object ID），`SOUL_B_ID`（Soul B on-chain ID），`COLLECTION_ID`（Collection on-chain ID）
...
7. `evaluate_script` 验证 "Standard" royalty (5%) 默认推荐选中（按钮含 `border-purple` class）
...
4. `click` "Collections" tab → 验证 "No collection rights yet"
5. `click` "Activity" tab → 验证 "No activity yet"
...
8. `click` "Standard" royalty 按钮设置 5%
...
### Test 5.2: Issue SoulGrant to Agent Alpha（seal + memory scope）

前置：`SOUL_A_STATE_OBJ` 已在 Test 1.8 从 Soul A 详情页的 Protocol State 卡片记录。

需要 `__e2eIssueGrant` 函数，该函数仅在 `/create/gas` 页面挂载（需 CreateSoulProvider context）。先走 create wizard 填最小数据到达 gas 页：

1. `navigate_page` → `http://localhost:3100/create`
2. Quick-fill: name `_e2e_grant_helper`, description `temp`
3. `upload_file` cover ← `/Users/admin/Documents/example/images.jpeg`
4. `click` "Next: Living Content →"
5. 打标签 + `upload_file` char ← `soul.md`, memory ← `memory.md`
6. `click` "Next: Soul Awakened →" → `click` "Next: Pay Gas →"
7. `wait_for` gas 页加载（`__e2eIssueGrant` 可用）
8. `evaluate_script` 调用 grant:
   ```javascript
   evaluate_script(`
     window.__e2eIssueGrant({
       stateObjectId: '${SOUL_A_STATE_OBJ}',
       granteeAddress: '0x3b82...8610',
       scopeMask: 3,
       soulObjectId: '${SOUL_A_ID}'
...
11. `take_screenshot` → `e2e/phase5-grant-issued.png`
...
### Test 5.6: Revoke SoulGrant
1. 重新导航到 `/create/gas`（通过快速填 wizard 或复用已打开的 gas 页）
2. `evaluate_script` 调用 revoke:
   ```javascript
   evaluate_script(`
     window.__e2eRevokeGrant({
       stateObjectId: '${SOUL_A_STATE_OBJ}',
       granteeAddress: '0x3b82...8610',
       soulObjectId: '${SOUL_A_ID}'
     }).then(r => JSON.stringify(r))
   `)
   ```
3. 刷新 Soul A 详情页 → 验证 Active Grants 恢复 "No active SoulGrant"
4. `take_screenshot`
...
### Test 6.3: Memory Entry Append
1. **当前限制**：仓库目前只暴露了 `__e2eUpload` / `__e2eIssueGrant` / `__e2eRevokeGrant` 等 helper，**未暴露** `__e2eAppendMemory` 或通用签名 helper；仅靠当前 Chrome DevTools MCP 步骤无法直接构建并提交 owner memory append TX。
2. 若本轮先补出 `__e2eAppendMemory`（或等效本地脚本），执行顺序必须是：
   - 先上传新的 memory 文件，拿到 `contentBlobObjectId` 和 `rawMemoryEnvelope`
   - 再构建 `append_as_owner(memoryOnChainId, stateOnChainId, contentBlobObjectId)` TX
   - 签名成功后 `POST /api/souls/${SOUL_A_ID}/memory`，body 至少包含 `txDigest`，私有 memory 还应带上 `rawMemoryEnvelope`
   - 刷新详情页，验证 "Recent Memory" 出现新增 entry row，且 `writerKind` 为 `owner`
3. 若未补 helper，本轮只做 "Recent Memory" 面板渲染 smoke，不把 append 记为通过
...
## Phase 7: Agent API 功能验证（6 tests）
...
### Test 7.6: Agent Seal Decrypt Soul B
```bash
SOUL_ID=${SOUL_B_ID} \
AGENT_API_KEY="${E2E_AGENT_ALPHA_API_KEY}" \
AGENT_MNEMONIC="${E2E_AGENT_ALPHA_MNEMONIC}" \
...
### Test 10.2: Resources — Content Format Page
...
Tests 1.6-1.7 (create Soul A + B, both held) → SOUL_A_ID, SOUL_B_ID
...
| 2 | 6 | 上架 Soul A ($1) + Soul B ($2) |
| 3 | 5 | Collection 创建（seller session 内） |
| 4 | 6 | Buyer 登录 + 购买 Soul A |
| 5 | 7 | Grant 发放 / 验证 / 撤销（含 Agent access 验证） |
| 6 | 4 | Skills append + Memory + 解密 |
| 7 | 6 | Agent API 全流程（search → detail → purchase → access → decrypt） |
| 8 | 5 | Import 流程（source → upload → map → preview → gas → success） |
...
| Royalty Standard | `button:has-text("Standard")` (含 Recommended tag) | /create |
...
| 函数 | 签名 | 用途 |
|------|------|------|
| `__e2ePublish` | `(params: PublishParams) => Promise` | 触发 mint TX |
| `__e2eUpload` | `(fileContent: string, fileName: string, type?: 'public'\|'encrypted') => Promise<UploadResult>` | Walrus 文件上传 |
| `__e2eListSoul` | `(params: { currentKioskId, currentKioskCapOnChainId, stateObjectId, soulObjectId, priceAtomic }) => Promise` | 上架 |
| `__e2eGetAuthHeaders` | `() => Promise<Record<string, string>>` | 获取 auth headers |
| `__e2eIssueGrant` | `(params: { stateObjectId, granteeAddress, scopeMask, soulObjectId }) => Promise` | 发放 grant |
| `__e2eRevokeGrant` | `(params: { stateObjectId, granteeAddress, soulObjectId }) => Promise` | 撤销 grant |
...
| `new-web/app/create/gas/page.tsx` | 创建 Step 4 + E2E helpers — Phase 1.5-1.6, 5.2, 5.6 |
...
| `new-web/app/my-souls/page.tsx` | My Souls 4-tab — Phase 1.11, 4.6, 5.5 |
...
| `new-web/lib/hooks/use-grant.ts` | Grant hook — Phase 5.2, 5.6 |
...
| `new-web/scripts/e2e-agent-decrypt.ts` | Agent Seal 解密（`seal_approve_owner` / `seal_approve_granted_agent`） |
...
11. **Memory append helper 缺口**: 当前仓库未暴露 `__e2eAppendMemory` 或通用签名 helper；默认只将 Phase 6.3 记为 gated，不把它算进“可直接执行”集合。
...
默认验收口径：
- 63 项可直接执行测试通过
- Phase 6.3 明确标记为 gated，除非先补 `__e2eAppendMemory`（或等效本地脚本）后再执行
- 截图存档到 `docs/e2e-screenshots/`
- 测试结果更新到 `docs/e2e-test-results-new-web.md`
- Phase 11 cleanup 完成后 market 恢复空状态，DB 无残留
