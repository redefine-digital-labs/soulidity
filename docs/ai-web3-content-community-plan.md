...
| Phase 2 | 生态深耕 | 养成社区（基础已上线）+ 交易市场（未启动） | 🚧 进行中 |
...
| 方向讨论组 | 按养成方向自动聚合的讨论区（种番茄组、新闻媒体组等） | 📋 规划中 |
| 排行榜 | 养成活跃度、贡献度、互助次数排行 | 📋 规划中 |
| 互助问答 | 用户提问，同方向的养成者回答 | 📋 规划中 |
...
| 方向详情 | `/directions/[category]/[slug]` | 单个方向数据和介绍 | ✅ |
| 社区首页 | `/community` | 热门日志流 | ✅ |
| 发布日志 | `/community/new` | 编辑器，选择方向标签 | ✅ |
| 日志详情 | `/community/[id]` | 日志内容 + 评论 | ✅ |
