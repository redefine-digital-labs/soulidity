...
export default function TweetsReviewPage() {
  ...
  async function fetchItems() {
    ...
    else if (res.status === 401 || res.status === 403) { setAuthError(true) }
    setLoading(false)
  }
  ...
  if (authError) return <div className="max-w-5xl mx-auto px-6 py-8" style={{ color: 'var(--text-muted)' }}>无权限访问此页面</div>
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-2 shrink-0 items-end">
                    <input
                      ...
                      onChange={e => setTweetTags(prev => ({ ...prev, [item.id]: e.target.value }))}
                      className="input-dark text-xs w-48"
                    />
                    <div className="flex gap-2">
                      <button onClick={() => handleApprove(item.id)} disabled={isProcessing || !hasTags} className="btn btn-success text-xs">{isProcessing ? '处理中...' : '通过'}</button>
