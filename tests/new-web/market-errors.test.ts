...
describe('market error catalog', () => {
  it('covers every active error code on market.move (0-42, except admin slot 17)', () => {
    const expected = [
      ...Array.from({ length: 17 }, (_, i) => i), // 0..16
      ...Array.from({ length: 25 }, (_, i) => i + 18), // 18..42
    ]
  ...
  it('reserves slot 17 (no entry) since market.move skips it', () => {
  ...
  it('keeps the four split target codes mapped to their narrowed semantics', () => {
    ...
    expect(MARKET_ERROR_CATALOG[18]?.name).toBe('EStateMismatch')
  })
...
describe('getMarketAbortInfo', () => {
  it('reads enhanced market abort metadata for assets-root retry decisions', () => {
    const original = new Error(
      "Transaction resolution failed: MoveAbort in 1st command, abort code: 34, in '0xab::market::init_assets_and_append_sprite_as_owner' (instruction 9)",
    )
    const enhanced = enhanceMarketError(original)
    const info = getMarketAbortInfo(enhanced)

    expect(info?.code).toBe(34)
    expect(info?.entry.name).toBe('EAssetsRootAlreadyExists')
    expect(info?.functionName).toBe('init_assets_and_append_sprite_as_owner')
  })

  it('falls back through an error cause when wrapper metadata is absent', () => {
    const cause = new Error(
      "Transaction resolution failed: MoveAbort in 2nd command, abort code: 36, in '0xab::market::init_skills_and_append_skill_as_owner' (instruction 11)",
    )
