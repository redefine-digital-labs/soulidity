...
const mockedExtractGrantCapacityUpdatedEvent = vi.hoisted(() => vi.fn())
...
vi.mock('@/lib/soulidity/events', () => ({
  extractGrantCapacityUpdatedEvent: mockedExtractGrantCapacityUpdatedEvent,
}))
...
function makeRequest(body: Record<string, unknown> = { txDigest: TX_DIGEST }) {
  return new Request(`http://localhost/api/souls/${STATE_ID}/grant-capacity`, {
    method: 'POST',
...
describe('POST /api/souls/[id]/grant-capacity', () => {
  beforeEach(() => {
    ...
    mockedReadTransactionSender.mockReturnValue(OWNER_ADDRESS)
    mockedAssertTransactionSender.mockReturnValue(null)
    mockedExtractGrantCapacityUpdatedEvent.mockReturnValue({
      soulId: SOUL_ID,
      oldCapacity: 1,
      newCapacity: 2,
    })
  ...
  async function callRoute(body?: Record<string, unknown>) {
    const { POST } = await import('../../web/app/api/souls/[id]/grant-capacity/route.ts')
    return POST(makeRequest(body) as never, { params: Promise.resolve({ id: STATE_ID }) })
  }

  it('mirrors grant capacity from the verified chain transaction', async () => {
    ...
    expect(mockedFindSoulAssetDetailByRouteId).toHaveBeenCalledWith(STATE_ID)
    expect(mockedSyncSoulProjectionFromChain).toHaveBeenCalledWith({
  ...
  it('returns 422 when the transaction updated a different soul', async () => {
    mockedExtractGrantCapacityUpdatedEvent.mockReturnValueOnce({
    ...
    await expect(response.json()).resolves.toEqual({
      error: 'Transaction updated grant capacity for a different Soul',
    })
